...
- Bug when restoring serialized files generated by ``openmmtools<0.21.3``. Fixed by catching the ``KeyError`` exception when deserializing. Issue `#612 <https://github.com/choderalab/openmmtools/issues/612>`_, PR `#613 <https://github.com/choderalab/openmmtools/pull/613>`_.
- Bug in generating real time analysis output when reporter is in the same working directory. Fixed by using ``os.path.join`` for creating the output path. Issue `#615 <https://github.com/choderalab/openmmtools/issues/615>`_, PR `#616 <https://github.com/choderalab/openmmtools/pull/616>`_.
