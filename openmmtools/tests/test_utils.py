...
def test_platform_supports_precision():
    ...
    for platform_index in range(openmm.Platform.getNumPlatforms()):
        ...
        supported_precisions = {
            ...
            if platform_supports_precision(platform, precision)
        }
        if platform_name == "Reference":
            if supported_precisions != {"double"}:
                raise Exception(
                    f"'Reference' platform should only support 'double' precision, but platform_supports_precision reports {supported_precisions}"
                )
        if platform_name == "CUDA":
            if supported_precisions != {"single", "mixed", "double"}:
                raise Exception(
                    f"'CUDA' platform should support 'mixed' precision, but platform_supports_precision reports {supported_precisions}"
                )
        if platform_name == "CPU":
            if supported_precisions != {"mixed"}:
                raise Exception(
                    f"'CPU' platform should support 'mixed' precision, but platform_supports_precision reports {supported_precisions}"
                )
...
def test_sanitize_expression():
    ...
    for word in _RESERVED_WORDS_PATTERNS:
        ...
        for expression, result in test_cases[word]:
            sanitized_expression, sanitized_variables = sanitize_expression(
...
def test_math_eval():
    ...
    test_cases = [
        ...
        (
            "(myset or my2set) & myset3",
            {"myset": {1, 2}, "my2set": {3, 4}, "myset3": {2, 3}},
            {2, 3},
        ),
    ]
...
def test_is_quantity_close():
    ...
    test_cases = [
        (300.0 * unit.kelvin, 300.000000004 * unit.kelvin, True),
...
def test_quantity_from_string():
    ...
    test_strings = [
        ...
        (
            '" 0.3 * kilojoules_per_mole / watt**3"',
            0.3 * unit.kilojoules_per_mole / unit.watt**3,
        ),  # Handle division, exponent, nested string
        (
            "1*meter / (4*second)",
            0.25 * unit.meter / unit.second,
        ),  # Handle compound math and parenthesis
        (
            "1 * watt**2 /((1* kelvin)**3 / gram)",
            1 * (unit.watt**2) * (unit.gram) / (unit.kelvin**3),
        ),  # Handle everything
...
class TestRestorableOpenMMObject:
    ...
    def setup_class(cls):
        ...
        class DummyRestorableCustomForce(
            RestorableOpenMMObject, openmm.CustomBondForce
        ):
            def __init__(self, *args, **kwargs):
                super().__init__(*args, **kwargs)

        class DummyRestorableCustomIntegrator(
            RestorableOpenMMObject, openmm.CustomIntegrator
        ):
            def __init__(self, *args, **kwargs):
                super().__init__(*args, **kwargs)
    ...
    def test_restorable_openmm_object(self):
        ...
        for openmm_object, is_restorable in test_cases:
            ...
            assert (
                RestorableOpenMMObject.restore_interface(openmm_object) is is_restorable
            ), err_msg
    ...
    def test_restorable_openmm_object_hash_collisions(self):
        ...
        restorable_classes_names = {
            restorable_cls.__name__ for restorable_cls in restorable_classes
        }
