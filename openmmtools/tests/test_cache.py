...
class TestContextCache(object):
    ...
    def test_platform_properties(self):
        ...
        assert "All platform properties must be strings." in str(cm.value)
        # non-dict properties
        with pytest.raises(ValueError) as cm:
            ContextCache(platform=cpu_platform, platform_properties="jambalaya")
        assert str(cm.value) == "platform_properties must be a dictionary"
        # invalid property
        with pytest.raises(ValueError) as cm:
            ContextCache(platform=cpu_platform, platform_properties={"jambalaya": "2"})
        assert "Invalid platform property for this platform." in str(cm.value)
