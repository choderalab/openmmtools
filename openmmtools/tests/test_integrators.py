...
def get_all_custom_integrators(only_thermostated=False):
    ...
    if only_thermostated:
        ...
        predicate = lambda x: old_predicate(x) and issubclass(
...
def check_stability(
    integrator, test, platform=None, nsteps=100, temperature=300.0 * unit.kelvin
):
    """
    ...
    if np.isnan(potential):
        raise Exception(
            "Potential energy for integrator %s became NaN." % integrator.__doc__
        )
...
def check_integrator_temperature(integrator, temperature, has_changed):
    ...
    assert np.isclose(
        ...
        kT.value_in_unit_system(unit.md_unit_system),
    )
    assert np.isclose(
        integrator.kT.value_in_unit_system(unit.md_unit_system),
        kT.value_in_unit_system(unit.md_unit_system),
    )
...
def test_stabilities():
    ...
    test_cases = {
        "harmonic oscillator": ts.HarmonicOscillator(),
...
def test_nose_hoover_integrator():
    ...
    num_dof = (
        3 * testsystem.system.getNumParticles() - testsystem.system.getNumConstraints()
    )
    ...
    for n in range(100):
        ...
        temp = (2.0 * kinE / (num_dof * unit.MOLAR_GAS_CONSTANT_R)).value_in_unit(
            unit.kelvin
        )
    ...
    assert (
        abs(mean_temperature - temperature.value_in_unit(unit.kelvin)) < 10.0
    ), mean_temperature
...
def test_pretty_formatting():
    ...
    for integrator_name, integrator_class in custom_integrators:
        ...
        if hasattr(integrator, "pretty_format"):
            ...
            msg = (
                "integrator.pretty_format(as_list=True) has %d lines while integrator has %d steps"
                % (len(lines), integrator.getNumComputations())
            )
...
def test_update_context_state_calls():
    ...
    for integrator_name, integrator_class in custom_integrators:
        ...
        msg = (
            "Integrator '%s' has %d calls to addUpdateContextState(), while there should be only one."
            % (integrator_name, num_force_update)
        )
        if hasattr(integrator, "pretty_format"):
...
def test_vvvr_shadow_work_accumulation():
    ...
    context.setVelocitiesToTemperature(temperature)
    assert (
        integrator.get_shadow_work(dimensionless=True) == 0
    ), "Shadow work should initially be zero."
    assert (
        integrator.get_shadow_work() / unit.kilojoules_per_mole == 0
    ), "integrator.get_shadow_work() should have units of energy."
    assert (
        integrator.shadow_work / unit.kilojoules_per_mole == 0
    ), "integrator.shadow_work should have units of energy."
    integrator.step(25)
    assert (
        integrator.get_shadow_work(dimensionless=True) != 0
    ), "integrator.get_shadow_work() should be nonzero after dynamics"
...
def test_baoab_heat_accumulation():
    ...
    context.setVelocitiesToTemperature(temperature)
    assert (
        integrator.get_heat(dimensionless=True) == 0
    ), "Heat should initially be zero."
    assert (
        integrator.get_heat() / unit.kilojoules_per_mole == 0
    ), "integrator.get_heat() should have units of energy."
    assert (
        integrator.heat / unit.kilojoules_per_mole == 0
    ), "integrator.heat should have units of energy."
    integrator.step(25)
    assert (
        integrator.get_heat(dimensionless=True) != 0
    ), "integrator.get_heat() should be nonzero after dynamics"
...
def test_external_protocol_work_accumulation():
    ...
    assert (
        integrator.get_protocol_work(dimensionless=True) == 0
    ), "Protocol work should be 0 initially"
    assert (
        integrator.get_protocol_work() / unit.kilojoules_per_mole == 0
    ), "Protocol work should have units of energy"
    integrator.step(1)
    assert (
        integrator.get_protocol_work(dimensionless=True) == 0
    ...
    assert (
        integrator.get_protocol_work(dimensionless=True) != 0
    ), "There should be protocol work after perturbing."
    assert integrator.protocol_work == (
        pe_2 - pe_1
...
class TestExternalPerturbationLangevinIntegrator(TestCase):
    def create_system(
        ...
        if platform_name in ["CPU", "CUDA"]:
            try:
                ...
                if (
                    platform_name == "CPU"
                    and openmm.version.short_version < mm_min_version
                ):
                    print(
                        f"Deterministic CPU forces not present in versions of OpenMM prior to {mm_min_version}"
                    )
                else:
                    raise e
    ...
    def test_initial_protocol_work(self):
        ...
        testsystem.system.addForce(
            openmm.MonteCarloBarostat(1 * unit.atmospheres, temperature, 2)
        )
        context, integrator = self.create_system(
            testsystem, parameter_name, parameter_initial, temperature, platform_name
        )
    ...
    def test_reset_protocol_work(self):
        ...
        testsystem = testsystems.AlchemicalAlanineDipeptide(
            nonbondedMethod=getattr(app, nonbonded_method)
        )
        context, integrator = self.create_system(
            testsystem, parameter_name, parameter_initial, temperature, platform_name
        ...
        for i in range(niterations):
            ...
            integrator.reset()
            external_protocol_work, integrator_protocol_work = self.run_ncmc(
                context,
                integrator,
                temperature,
                nsteps,
            ...
            assert abs(external_protocol_work - integrator_protocol_work) < 1.0e-5
    ...
    def test_ncmc_update_parameters_in_context(self):
        ...
        wbox = testsystems.WaterBox(
            box_edge=size * unit.angstrom,
        ...
        def switchoff(omm_force, omm_context, frac=0.9):
            omm_force.setParticleParameters(
                0,
                charge=-0.834 * frac,
            ...
            omm_force.setParticleParameters(
                1, charge=0.417 * frac, sigma=0, epsilon=1 * frac
            )
            omm_force.setParticleParameters(
                2, charge=0.417 * frac, sigma=0, epsilon=1 * frac
            )
            omm_force.updateParametersInContext(omm_context)

        def switchon(omm_force, omm_context):
            omm_force.setParticleParameters(
                0, charge=-0.834, sigma=0.3150752406575124, epsilon=0.635968
            )
    ...
    def test_protocol_work_accumulation_waterbox(self):
        ...
        for nonbonded_method in ["CutoffPeriodic"]:
            testsystem = testsystems.AlchemicalWaterBox(
                nonbondedMethod=getattr(app, nonbonded_method),
                box_edge=12.0 * unit.angstroms,
                cutoff=5.0 * unit.angstroms,
            )
            for platform_name in platform_names:
                name = "{} {} {}".format(
                    testsystem.name, nonbonded_method, platform_name
                )
                self.compare_external_protocol_work_accumulation(
                    testsystem,
                    parameter_name,
    ...
    def test_protocol_work_accumulation_waterbox_barostat(
    ...
    def compare_external_protocol_work_accumulation(
        ...
        for step in range(nsteps):
            ...
            self.assertAlmostEqual(
                external_protocol_work, integrator_protocol_work, msg=message
            )
...
def test_temperature_getter_setter():
    ...
    for integrator_name, integrator_class in custom_integrators:
        ...
        check_integrator_temperature_getter_setter.description = (
            "Test temperature setter and " "getter of {}"
        ).format(integrator_name)
...
def run_alchemical_langevin_integrator(nsteps=0, splitting="O { V R H R V } O"):
    ...
    alchemical_functions = {
        "forward": {
            name: "(1-lambda)*{:f} + lambda*{:f}".format(
    ...
    for direction in directions:
        ...
        for iteration in range(n_iterations):
            ...
            assert current_lambda == 0.0, (
                "initial lambda should be 0.0 (was %f)" % current_lambda
            )
            current_step = nonequilibrium_integrator.getGlobalVariableByName("step")
            assert current_step == 0.0, (
                "initial step should be 0 (was %f)" % current_step
            )

            compound_integrator.step(
                max(1, nsteps)
            )  # need to execute at least one step
            work[direction][iteration] = nonequilibrium_integrator.get_total_work(
                dimensionless=True
            )
            ...
            assert (
                current_lambda == 1.0
            ), "final lambda should be 1.0 (was {:f}) for splitting {}".format(
                current_lambda, splitting
            )
            current_step = nonequilibrium_integrator.getGlobalVariableByName("step")
            assert int(current_step) == max(1, nsteps), (
                "final step should be %d (was %f) for splitting %s"
                % (max(1, nsteps), current_step, splitting)
            )
            nonequilibrium_integrator.reset()
...
def test_periodic_langevin_integrator(
    ...
    for cycle in range(2):
        ...
        for i in range(nsteps_neq):
            ...
            assert np.isclose(
                integrator.getGlobalVariableByName("lambda"), (i + 1) / nsteps_neq
        ...
        for i in range(nsteps_neq):
            ...
            assert np.isclose(
                integrator.getGlobalVariableByName("lambda"), 1 - (i + 1) / nsteps_neq
            )
